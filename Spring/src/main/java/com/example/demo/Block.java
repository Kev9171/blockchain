...
public class Block {
	...
	public Document BlockDocument(int nonce) throws JSONException{//ÈÄº¸ÀÚ ºí·Ï
		...
		for(Document t: tx_docs){
			txs.put(new JSONObject(JSON.serialize(t)));
			//txs.put(new JSONObject(t.toJson()));
			//System.out.println("±Ü¾î¿È"+JSON.serialize(t));
		}
		
		System.out.println("testing" + txs);

		block.put("prevBlockHash", calculateHash(Lastblock()));
		block.put("nonce", nonce);
		block.put("timestamp", Instant.now().toEpochMilli());
		block.put("txs", txs);
	...
	public String calculateHash(Document d) throws JSONException{
		...
		//System.out.println("JSON : "+MerkleRootHash.getMerkleRoot(new JSONObject(JSON.serialize(d))));
		//System.out.println("ÀÏ¹Ý : "+(String)d.get("merkleRootHash"));

		String calculatedhash = sha.sha(
				(String)d.get("prevBlockHash") + 
				//(String)d.get("merkleRootHash") +
				merkleroothash.getMerkleRoot(new JSONObject(JSON.serialize(d))) +
