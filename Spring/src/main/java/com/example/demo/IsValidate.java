	...
	public boolean IsValidInputs(JSONObject json) throws JSONException{
		...
		for(int i=0;i<inputs.length();i++){
			...
			for(int j=0;j<utxo.length();j++){
				System.out.println("utxo id : "+utxo.getJSONObject(j).getString("id"));
				System.out.println("input id : "+inputs.getJSONObject(i).getString("id"));
				System.out.println(utxo.getJSONObject(j).getString("id")==inputs.getJSONObject(i).getString("id"));
				...
				if(utxo.getJSONObject(j).getString("id").equals(inputs.getJSONObject(i).getString("id"))){
					...
					for(int k=0;k<outputs.length();k++){
						System.out.println("ouptus ID : "+outputs.getJSONObject(k).getString("receiver"));
						System.out.println("sender ID : "+tx.getString("sender"));
						System.out.println("ouptus amount : "+outputs.getJSONObject(k).getInt("amount"));
						System.out.println("inputs amount : "+tx.getString("sender"));
